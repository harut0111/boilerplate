...
const Converter = () => {
    ...
    const options1 = quoteList.map((item: string, index: number) => {
        return item !== to ? <option value={`${item}`}  key= {index}>{item}</option> : null 
    })

    const options2 = quoteList.map((item: string, index: number) => {
        return item !== from ? <option value={`${item}`}  key= {index}>{item}</option> : null
    })
        ...
        <div className='home-convertor'>
            ...
            <div className='home-convertor-content'>
                ...
                <div className='home-convertor-result'>
                    <span>Итого</span>
                    <span>{ quoteVal ? amount * quoteVal : <span style={{color: 'red'}}>котировка не доступна</span>}</span>
                </div>
            </div>
        </div>
    )
